...
router.delete('/:id', (req: AuthRequest, res: Response) => {
  ...
  // Check if coffee bean is used in any brews
  const brewCount = db.prepare(
    'SELECT COUNT(*) as count FROM brews WHERE coffee_bean_id = ? AND user_id = ?'
  ).get(id, userId) as { count: number };
  
  if (brewCount.count > 0) {
    return res.status(400).json({ 
      error: `Cannot delete this coffee bean. It is used in ${brewCount.count} brew(s). Please delete those brews first.` 
    });
  }
  
  // Batches will be deleted automatically due to ON DELETE CASCADE
  db.prepare('DELETE FROM coffee_beans WHERE id = ? AND user_id = ?').run(id, userId);
  res.json({ success: true });
});
