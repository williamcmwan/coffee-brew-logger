...
export default function BrewHistory() {
  ...
  const getDaysAfterRoast = (batchId: string, beanId: string, brewDate: string) => {
    const bean = coffeeBeans.find(b => b.id === beanId);
    if (!bean) return null;
    const batch = bean.batches.find(b => b.id === batchId);
    if (!batch || !batch.roastDate) return null;
    const roastDate = new Date(batch.roastDate);
    const brew = new Date(brewDate);
    const diffTime = brew.getTime() - roastDate.getTime();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({
        title: "Brew deleted",
        description: "Brew log has been removed successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
